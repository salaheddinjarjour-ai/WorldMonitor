...
// Use current origin for BASE_URL - works on any domain
...
export function updateMetaTagsForStory(meta: StoryMeta): void {
  ...
  const title = `${countryName} Intelligence Brief | Global Intelligence Monitor`;
...
export function resetMetaTags(): void {
  const defaultTitle = 'Global Intelligence Monitor - Real-time Global Insights';
...
function generateDescription(
  ...
  return `Global Intelligence Monitor ${parts.join('. ')}. Free, open-source geopolitical intelligence.`;
}
